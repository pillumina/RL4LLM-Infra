...
起点是 **GQA 8:1**——8 个 Query 头共享 1 个 KV head，缓存直接缩小 8 倍。但这显然会损失信息容量。Google 的应对是 **Key 维度翻倍**：更宽的 KV 向量能在更少的 head 数量下保留足够的信息。在这个基础上，进一步让 **K = V**——直接让 Key 向量等于 Value 向量，KV Cache 再减半。这强制模型在"检索"（Key 的角色）和"读取"（Value 的角色）之间使用同一套表示，降低了表示的自由度，但也降低了过拟合的风险。
...
| 配置项 | Gemma 4 26B-A4B | Qwen3-235B-A22B | GLM-5 |
|--------|-----------------|-----------------|-------|
| 总参数 | 25.2B | 235B | 744B |
| 专家总数 | 128 | 128 | 256 |
| 每token激活 | 8 + 1 共享 | 8（无共享） | 8 + 1 共享 |
| 活跃参数 | 3.8B | 22B | 40B |
| GQA | 32Q / 4KV | 64Q / 4KV | MLA（无传统 KV） |
| 层数 | — | 94 层 | — |
| 上下文 | 256K | 128K（原生） | 200K |
