...
> 2026 年 4 月 2 日，Google DeepMind 发布 Gemma 4。31B 参数的 dense 模型在 AIME 2026 上拿到 89.2%，MoE 变体 26B-A4B 以 3.8B 活跃参数跑出 31B dense 模型 97% 的性能。与此同时，Qwen3-235B 的总参数量是 31B 的 7.6 倍，GLM-5 更是达到 744B 总参数。
...
Gemma 4 在 sliding window 层使用标准 RoPE（`rope_theta=10000`），在 global 层使用 p-RoPE（`partial_rotary_factor=0.25`, `rope_theta=1000000`）。这不是随意的选择——sliding window 的上下文窗口较短（边缘模型 E2B/E4B 为 512 token，31B/26B-A4B 为 1024 token，均来自 config.json 的 `sliding_window` 字段），标准 RoPE 完全够用。但 global 层要处理 256K 的完整上下文，标准 RoPE 的低频分量在这个尺度上会变得不可靠。分开处理让每一层都使用最适合自己上下文范围的位置编码。
...
26B-A4B 是 Gemma 4 家族中最值得深入分析的模型。25.2B 总参数，每 token 激活约 3.8B，性能达到 31B dense 模型的 97%。
...
层交替使用 local sliding window（1024 token）和 global full-context attention（5:1 交错比例，从 `layer_types` 验证），通过 K=V、GQA 8:1、p-RoPE 将 global 层开销压缩到极致。设计目标是**在固定显存预算内最大化上下文长度**。
...
| Qwen3 | 标准 RoPE (θ=5M) + ABF | base frequency 5M，max_position 256K |
