...
Gemma 4 系列提供多模态变体（视觉 + 音频），与文本模型分开发布。视觉编码器基于 ViT，使用 2D RoPE 编码 patch 的二维空间位置，支持可变宽高比和可配置的 soft token budget（70-1120 tokens）。音频编码器沿用 Gemma-3n 的 USM-style conformer，从 681M 压缩到 305M。视觉编码器基于 ViT，使用 2D RoPE 编码 patch 的二维空间位置，支持可变宽高比和可配置的 soft token budget（70-1120 tokens）。音频编码器沿用 Gemma-3n 的 USM-style conformer，从 681M 压缩到 305M。
