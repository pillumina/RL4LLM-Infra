...
| 多模态 | ❌ (独立变体) | ✅ 原生融合 | ❌ (VL 分离) | ❌ (tool-calling) |
...
### 1.6 同规模 MoE 的两种参数分配哲学

Gemma 4 26B-A4B 和 Qwen3.5-35B-A3B 的活跃参数接近（3.8B vs 3B），但分配策略截然不同。从 HuggingFace config.json 可以量化这种差异：

**每层活跃 FFN 容量：** Gemma 4 的 8 个专家（每个 704 维）加上必须执行的 dense MLP（2112 维），总计 8×704 + 2112 = 7744。Qwen3.5 的 8 个专家（每个 512 维）加上 shared expert（512 维），总计 8×512 + 512 = 4608。Gemma 4 每层 FFN 容量是 Qwen3.5 的 1.68 倍。

但 Qwen3.5 有 40 层 vs Gemma 4 的 30 层。总活跃 FFN 容量：Gemma 4 为 7744×30 = 232,320，Qwen3.5 为 4608×40 = 184,320。Gemma 4 仍多 26%。加上 attention 参数（Gemma 4 的 8 KV heads vs Qwen3.5 的 2 KV heads），活跃参数 3.8B vs 3B 的差距就合理了。

**注意力成本结构的差异同样显著。** Gemma 4 每层都是标准 O(n²) 注意力，8 KV heads 意味着更大的 KV Cache。Qwen3.5 的 3/4 层使用 linear attention（从 `layer_types` 验证），计算复杂度从 O(n²) 降到近 O(n)，完全消除 KV Cache。这意味着：

- Gemma 4 每层活跃参数多 + 注意力成本高 + KV Cache 大 → **每层表达力强，但贵**
- Qwen3.5 每层活跃参数少 + 3/4 层注意力极便宜 + 无 KV Cache → **每层表达力弱，但便宜且更深**

两种策略在相近的活跃参数预算下做出了截然不同的取舍。仅有的直接可比数据（MMLU-Pro）显示 Qwen3.5-35B-A3B 以更少活跃参数（3B vs 3.8B）小幅领先（85.3% vs 82.6%），但一个 benchmark 远不足以定论。两种策略的实际优劣，需要更多交叉 benchmark 才能判断。
...
蒸馏不是 Gemma 4 的独门武器——四个团队都在做，而且都明确承认蒸馏优于纯 RL。
...
四个团队都在做蒸馏，关键差异在于 **Teacher 模型的能力上限**。Google 拥有 Gemini 3 作为 teacher。作为 Gemini 系列的第三代旗舰闭源模型，其能力在公开 benchmark 上显著优于任何开源模型，信息质量上限天然高于 Qwen3-235B 或 GLM-5 自蒸馏。但这只是基于公开信息的推断——Google 没有公开 Gemma 4 蒸馏数据的具体来源和规模，这是分析中的一个已知盲区。
...
**Gemma 4**：Google 未公开训练数据量。从 WaveSpeed 的分析文章到 HuggingFace 的技术博客，均未提及具体 token 数。这与其他三个团队主动公开 28.5T/36T 形成对比，是分析中的一个信息缺口。
### 2.3 QAT：训练时就考虑量化
...
Gemma 4 系列提供多模态变体（视觉 + 音频），与文本模型分开发布。视觉编码器基于 ViT，使用 2D RoPE 编码 patch 的二维空间位置，支持可变宽高比和可配置的 soft token budget（70-1120 tokens）。音频编码器沿用 Gemma-3n 的 USM-style conformer，从 681M 压缩到 305M。
...
Qwen3 采用最保守的注意力方案——每层都是标准 Full Attention，没有稀疏化或混合注意力。上下文扩展依赖 YARN（注意力分数中加入距离衰减）和 DCA（将序列切分为 chunk，chunk 内完整自注意，chunk 间通过 successive-chunk attention 传递信息），从训练时的 4K 无损扩展到 256K。这种方案最成熟、工具链支持最好，但在超长上下文下的计算成本是四者中最高的。
